$(document).ready(function() {
  ...
  //creating New-Tweets
  const errorNoWords = $('.noWords-error');
  const errorTooManyWords = $('.tooManyWords-error');
  const textarea = $('.new-tweet');
  const composeBtn = $('#nav-bar .compose');
  ...
  //validations before submitting
    if (content === "" || content === null) {
      errorNoWords.css({"opacity": "1"})
    } else if (textVal > 140) {
      errorTooManyWords.css({"opacity": "1"})
    } else {
      $.post( "/tweets", newTweet)
        ...
        loadTweets();
        textarea.val('');
        errorNoWords.css({"opacity": "0"});
        errorTooManyWords.css({"opacity": "0"});
      });
    }
  })
  loadTweets();
